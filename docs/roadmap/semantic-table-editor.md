...
- Component-level tokens. A Button's own `--btn-main` is still a local
  custom property filled from a role; a client that wants a token named
  for the component rather than for the role is a later layer.
