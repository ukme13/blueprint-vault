/*
 * Machado, Oliveira and Fernandes (2009), Table 1.
 *
 ...
 * November/December 2009, pp. 1291-1298.
 *
 * Supplementary matrices:
 * https://www.inf.ufrgs.br/~oliveira/pubs_files/CVD_Simulation/CVD_Simulation.html
 *
 * These numbers were extracted from that table by script rather than typed, so
 * that no digit passes through anybody's memory. Two properties of the
 * published data double as a check that the extraction stayed in step with the
 * table: severity 0.0 is exactly the identity, and every row of every matrix
 * sums to 1 within 1e-6. Both are asserted in the tests, and neither would
 * survive a misaligned parse.
 *
 * Row-major, and applied to LINEAR RGB. The paper's own equation (1) writes a
 * plain "RGB" vector, but the model is derived through cone fundamentals and
 * the transform is only meaningful on linear light. Applying it to
 * gamma-encoded sRGB is a common implementation error and makes a wide range
 * of colours come out too dark.
 *
 * Machado names these families protanomaly, deuteranomaly and tritanomaly:
 * anomalous trichromacy parameterised by severity, where severity 1.0 is the
 * dichromatic case. The short names below are the family, not the severity.
 *
 * Do not edit by hand. Re-extract from the page above if these ever need to
 * change.
 */
...
/** The cone family a matrix affects. Severity is carried separately. */
export type MachadoFamily = "protan" | "deutan" | "tritan";

/** Table 1 is tabulated at this step, from 0.0 to 1.0 inclusive. */
export const MACHADO_2009_SEVERITY_STEP = 0.1;

/** Eleven matrices per family, ordered by ascending severity. */
export const MACHADO_2009_MATRICES: Record<MachadoFamily, readonly Matrix3[]> =
  {
    protan: [
      // severity 0.0
