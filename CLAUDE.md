...
1. **Domain logic lives in `packages/ui`.** Pure data transformations, state
   mutations, and helpers belong there, not in `apps/`. A function that takes a
   value and returns a new one — `TypeSystem -> TypeSystem`, a formatter, a
   validator — is domain logic wherever it happens to be called from.
2. **Components render.** Aim for under 250 lines per component. Extract logic to
   `packages/ui` early rather than after a file has grown past reading.
   App-level hooks are for binding domain functions to React state, not for
   holding the logic itself.
3. **Import from package entry points.** Always `import { x } from "@blueprint/ui"`
   (which resolves to `packages/ui/src/index.tsx` via the package `exports`
   map). Never reach across a workspace with a deep or relative path.

## 🧪 Testing Strategy

1. **Vitest in `packages/ui`** for everything the package exports. `pnpm -r test`
   runs these. If logic is hard to unit test, that is usually a sign it is
   sitting in the wrong workspace.
2. **Playwright in `apps/`** for end-to-end user interactions only — not as a
   substitute for unit tests. E2E reaches the happy path; the edge cases belong
   in Vitest.
3. **A passing test proves nothing until it has failed.** When adding tests for
...
1. **Local first.** Always commit locally. Never `git push` or open a PR
   automatically.
2. **Push only when asked.** Push and create or update PRs only on an explicit
   request. Approval to push one branch does not carry to the next.
