...
- Prefer Astryx layout components and semantic HTML (`main`, `nav`, `section`,
  `header`, `ul`, and similar elements). A `<div>` is allowed for layout or
  grouping when no semantic element is appropriate. Full page → AppShell;
  sidebar nav → SideNav.
