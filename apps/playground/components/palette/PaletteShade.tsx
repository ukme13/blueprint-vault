...
export function PaletteShade({
    ...
    >
      <span className={styles.shadeContent}>
        ...
        {shade.isAnchor && (
          <span
            aria-label={
              shade.anchorType === "source" ? "Source colour" : "Colour anchor"
            }
  ...
  /* On a phone the details are in one sheet for the whole matrix
     (ShadeSheet), so the swatch only reports that it was picked. */
  if (!hasPopover) return swatch;
